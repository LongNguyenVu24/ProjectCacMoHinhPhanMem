...
public class daoPhieuKiemKeKho {
    ...
    public ArrayList<PhieuKiemKeKho> getListPhieuKiemKeKho() {
        ...
        try {
            ...
            while (rs.next()) {
                result.add(new PhieuKiemKeKho(rs.getInt("id_kk_kho"), rs.getInt("sl_hao_mon"), rs.getInt("sl_thuc_te"),
                        rs.getString("thoi_gian"), rs.getInt("id_kho"), rs.getInt("id_nv")));
            }
    ...
    public PhieuKiemKeKho getPhieuKiemKeKho(int id_kho) {
        ...
        try {
            ...
            ResultSet rs = DataProvider.getIntance().excuteQuery(query, arr);
            while (rs.next()) {
                result = (new PhieuKiemKeKho(rs.getInt("id_kk_kho"), rs.getInt("sl_hao_mon"), rs.getInt("sl_thuc_te"),
                        rs.getString("thoi_gian"), rs.getInt("id_kho"), rs.getInt("id_nv")));
            }
    ...
    public boolean insertPhieuKiemKeKho(int sl_hao_mon, int sl_thuc_te, String thoi_gian, int id_kho, int id_nv) {
        String query = "INSERT INTO `phieu_kiem_ke_kho`(`sl_hao_mon`,`sl_thuc_te`, `thoi_gian`, `id_kho`, `id_nv`) VALUES ('"
                + sl_hao_mon + "','" + sl_thuc_te + "','" + thoi_gian + "', '" + id_kho + "','" + id_nv + "')";
        ArrayList<Object> arr = new ArrayList<>();
        DataProvider.getIntance().open();
